...
func (ix *RuleIndex) AddRulesFromFile(c *config.Config, file *bf.File) {
	...
	for _, stmt := range file.Stmt {
		if call, ok := stmt.(*bf.CallExpr); ok {
			ix.addRule(call, c.GoPrefix, c.PrefixRoot, buildRel)
		}
	}
}

func (ix *RuleIndex) addRule(call *bf.CallExpr, goPrefix, prefixRoot, buildRel string) {
	...
	record := &ruleRecord{
		rule:  rule,
		label: label.New("", labelPath, rule.Name()),
	}
